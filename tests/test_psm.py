...
def test_psm_mint_and_redeem_apply_fees(protocol):
    ...
    assert protocol.stable_token.balance_of(address="alice") == pytest.approx(
        49
    )
    assert protocol.reserve_token.balance_of(address="alice") == pytest.approx(
        949.75
    )
    assert protocol.reserve_token.balance_of(
        address="treasury"
    ) == pytest.approx(1.25)
